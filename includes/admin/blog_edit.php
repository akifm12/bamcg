...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    $content = $_POST["content"]; // HTML from TinyMCE

    $thumbnailName = $blog["thumbnail"];

    if (!empty($_FILES["thumbnail"]["name"])) {

        if ($thumbnailName && file_exists("../uploads/blogs/" . $thumbnailName)) {
            unlink("../uploads/blogs/" . $thumbnailName);
        }
    ...
    $stmt = $pdo->prepare("
        ...
        WHERE id = ?
    ");
    $stmt->execute([$title, $slug, $content, $thumbnailName, $id]);
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    // 🔴 REQUIRED: sync editor content before submit
    ...
    <form method="post" enctype="multipart/form-data" class="space-y-6">
        ...
        <div>
            ...
            <?php if ($blog["thumbnail"]): ?>
                <img src="../uploads/blogs/<?= htmlspecialchars($blog["thumbnail"]) ?>"
                     class="h-24 rounded mb-3 border border-gray-700">
            <?php endif; ?>
        ...
        <div>
            ...
            <textarea name="content"
                      rows="12"
                      required
                      class="w-full bg-gray-800 border border-gray-700 px-4 py-3 rounded text-white"><?= htmlspecialchars($blog["content"]) ?></textarea>
        </div>
